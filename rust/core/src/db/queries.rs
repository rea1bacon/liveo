...
pub fn create_tracking_table_sql(track_table: &TrackTable) -> String {
    let table_name = format!(
        ...
        track_table.track_col.join("_"),
        track_table.on.to_string()
    );
    ...
    for col in &track_table.track_col {
        if track_table.old {
            columns.push(format!("old_{} TEXT", col))
        }
        if track_table.new {
            columns.push(format!("new_{} TEXT", col))
        }
    }
    ...
    format!(
        ...
        table_name.replace("\"", "\"\""), // Escape quotes
        columns_sql
    )
}
...
pub fn create_trigger_function_sql(track_table: &TrackTable) -> String {
    ...
    for col in &track_table.track_col {
        if track_table.old {
            columns.push(format!("old_{}", col));
            values.push(format!("OLD.{}", col));
        }
        if track_table.new {
            columns.push(format!("new_{}", col));
            values.push(format!("NEW.{}", col));
        }
    }
...
pub fn create_trigger_sql(track_table: &TrackTable) -> String {
    ...
    // Build trigger name and function name
    ...
    FOR EACH ROW 
    EXECUTE FUNCTION "{}"();"#,
        track_table.trigger_name(), // Escape quotes in trigger name
        timing,
        track_table.table_name(),
